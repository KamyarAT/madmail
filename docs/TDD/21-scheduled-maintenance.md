...
flowchart TB
    ...
    subgraph cli [Operator CLI]
        T[madmail tasks run / run-all / list]
    end
    MC --> SCH
    MC --> T
    Q --> P2
...
| `chatmail::ctl::tasks` | CLI `madmail tasks` |
...
madmail tasks run prune-unused-accounts --retention 720h
madmail tasks run prune-old-messages
madmail tasks run prune-unread-older --retention 48h
madmail tasks run renew-certificate   # manual renewal when tls_mode = autocert
```

Global flags: `--config`, `--state-dir` (same as other ctl commands). Requires `chatmail.db` / migrated state dir.

**systemd timer (optional):** run `madmail tasks run-all` on a timer instead of relying on the in-process scheduler when using `madmail run` only for protocol listeners — both are safe; jobs are idempotent.
...
- `maddy imap-acct prune-unused` → use `madmail tasks run prune-unused-accounts` (or `run-all`).
